...
class multi_dataset_summariser: # gathers information from datasets to produce as output
    ...
    def verify_success_of_all_datasets(self):
        ...
        for dataset_name,dataset in self.multi_dataset_summariser.items():
            ...
            dataset_name_success_dict[dataset_name] = len(summary_workflow_outputs)
        ...
        for dataset_name,dataset in self.multi_dataset_summariser.items():
            ...
            for output_name,output_val in dataset.summary_outputs.items():
                if output_name not in all_output_dict.keys():
                    all_output_dict[output_name] = [output_val]

                else:
                    all_output_dict[output_name].append(output_val)
...
class workflow_tools: # runs workflow on individual datasets
    ...
    def remove_chimeras(self, sample_name_and_path_dict, chimera_output_directory):
        ...
        for sample_name, fastq_path in sample_name_and_path_dict.items():
            ...
            new_file_path = "%s/chimera_removed_%s.fastq" % (chimera_output_directory, sample_name)


            new_fastq_paths.append(new_file_path)
            vsearch_chimera_args = ['vsearch', '--uchime_denovo', fastq_path, '--threads', str(self.configuration_dict['threads']), '--nonchimeras', new_file_path]

            p = Popen(vsearch_chimera_args, stdin=PIPE, stdout=PIPE, stderr=STDOUT)
    ...
    def convert_biom_to_tsv(self, biom_file):
        ...
        convert_args = ['biom', 'convert', '--to-tsv', '-i', biom_file, '-o', tsv_file]
    ...
    def convert_tsv_to_biom(self, tsv_file):
        ...
        subprocess.call(['biom', 'convert', '-i', tsv_file, '-o', biom_file, '--table-type=OTU table', '--to-json'])
        return biom_file



    def create_normalised_copy_number_table(self, filtered_biom):
        normalised_otu_table_biom = "%s/clustered_table_normalised_by_copy_number.biom" % (self.dataset_path)
        subprocess.call(['normalize_by_copy_number.py', '-i', filtered_biom, '-o', normalised_otu_table_biom, '-c', self.configuration_dict['copy_number_table_file']])
    ...
    def predict_metagenomes(self, normalised_biom, metagenome_prediction_dir):
        ...
        subprocess.call(['predict_metagenomes.py', '-i', normalised_biom, '-o', metagenome_predictions_biom, '-c', self.configuration_dict['ko_table']])

        metagenome_predictions_tsv = self.convert_biom_to_tsv(metagenome_predictions_biom)
    ...
    def categorize_metagenomes(self, metagenome_predictions_biom, ko_level, categorized_metagenome_dir):
        pathway_result = "%s/lvl_%s_collapsed_pathways.txt" % (categorized_metagenome_dir, ko_level)

        subprocess.call(['categorize_by_function.py', '-i', metagenome_predictions_biom, '-c', 'KEGG_Pathways', '-l', ko_level, '--format_tab_delimited', '-o', pathway_result])
...
class summary_tools:
    ...
    def chimera_result_summary(self, chimera_output_dict):
        sample_row_info = ["Sample\tNumber of sequences\tnumber of chimeras\tnumber of non chimeras"]
        chimera_info_list = []
        for sample_name, summary_info in chimera_output_dict.items(): # regex search for percentage chimera and non chimera and total sequences of read
        
            summary_info = summary_info.split("\n")
            found_both = [False, False]
            for info in summary_info:
                chimera_string_check = re.search("\A[0-9].*chimeras*", info)
                total_seq_string_check = re.findall(".*in.([0-9]*).*total sequences.", info)
                if chimera_string_check is not None:

                    chimera_string = chimera_string_check.string
                    chimera_info = re.findall("[(]{1}([^)]*)[)]{1}", chimera_string)
    ...
    def get_collapsed_pathway_summary(self, collapsed_pathway_dir, ko_pathway_levels):
        ...
        for lvl in ko_pathway_levels:
            try:
                pathway_table_file = "%s/lvl_%s_collapsed_pathways.txt" % (collapsed_pathway_dir, lvl)
                pathway_table = pd.read_csv(pathway_table_file, header=1,sep='\t', index_col=[0])
                self.summary_outputs['Pathways_at_lvl_%s' % (lvl)] = len(pathway_table.index.tolist())
            except:
...
class dataset(workflow_tools, summary_tools): # dataset object with fastq paths and attributes to be added etc.
    ...
    def run_workflow(self):
        ...
        merged_fastq_path = self.merge_fastqs_and_summarise(chimera_removed_fastq_paths)
    ...
    def chimera_removal_and_summarise(self):
        ...
        chimera_removed_fastq_paths, chimera_output_dict = self.remove_chimeras(sample_path_dict, chimera_output_directory)
        ...
        with open("%s/chimera_removal_directory/chimera_removal_summary.csv" % (self.dataset_path), "w") as f:
            f.write("sequence name\tinitial_total_sequences\ttotal_chimeras\ttotal_non_chimeras\n")
    ...
    def predict_and_categorise_metagenomes(self, normalised_biom):
        ...
        metagenome_predictions_biom,metagenome_prediction_table = self.predict_metagenomes(normalised_biom, metagenome_prediction_directory)
        ...
        for level in ko_pathway_levels:
            ...
            self.categorize_metagenomes(metagenome_predictions_biom, level, categorised_predictions_dir)
        
        self.get_collapsed_pathway_summary(categorised_predictions_dir, ko_pathway_levels)
